...
class IMAP{
    ...
    /**
     * Create a new coreIMAP instance.
     *
    ...
    public function __construct(){
        ...
        $this->Configurator = new Configurator('imap');

        // Initiate Logger
        $this->Logger = new Logger('imap');
                ...
                switch($option){
                    case"host":
                        if(is_string($value)){
                            ...
                            $this->Configurator->set('imap',$option, $value);
                        } else{
			...
			if(is_array($uids) && count($uids) > 0){
				foreach($uids as $uid){
					...
					if($count <= $limit || $limit === 0){
						...
						$message = new Message($uid, $message, $this->Connection, $directory);
						$messages[] = $message;
					} else {
						break;
					}
